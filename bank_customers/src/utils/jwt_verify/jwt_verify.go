...
func CustomerVerifyToken(r *http.Request) (*jwt.Token, error) {
	environ := godotenv.Load("/Users/klenam/Documents/go/src/bank_JWTauth/src/utils/.env")
	if environ != nil {
		log.Fatalf("Error loading .env file")
	}
	...
	token, err := jwt.Parse(tokenString, func(token *jwt.Token) (interface{}, error) {
		...
		return []byte(os.Getenv("C_ACCESS_SECRET")), nil
	})
	if err != nil {
		return nil, err
	}
...
func CustomerExtractTokenMetadata(r *http.Request) (*AccessDetails, error) {
	token, err := CustomerVerifyToken(r)
	if err != nil {
		return nil, err
	}
	claims, ok := token.Claims.(jwt.MapClaims)
